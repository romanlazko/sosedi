<x-app-layout>
    ...
    <div class="w-full space-y-6 m-auto max-w-2xl p-6">
        <x-form.post method="patch" action="{{ route('admin.telegram_bot.category.update', [$telegram_bot, $category]) }}">
            <x-white-block class="space-y-6 p-4">
                    ...
                    <x-form.label for="is_active" >
                        <div class="w-full flex items-stretch space-x-2">
                            <div class="flex-col">
                                ...
                                <x-form.input id="is_active" name="is_active" type="checkbox" class="mt-1 block" value="1" :checked="$category->is_active === 1"/>
                            </div>
                            <div class="flex-col">
                                {{ __('Is active:') }}
                            </div>
                ...
                <div>
                    ...
                    <x-form.input id="icon_name" name="icon_name" type="text" class="mt-1 block w-full" :value="old('icon_name', $category->icon_name)" required autofocus autocomplete="icon_name" />
                    <x-form.error class="mt-2" :messages="$errors->get('icon_name')" />
                </div>

                <livewire:translate-category :translations="$category->translations"/>
            </x-white-block>
